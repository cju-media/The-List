## 8-15-26
I missed yesterday. You win some you lose some.

Despite it being Saturday I managed to get *A LOT* done depsite having to take care of somethings while I was out of the house. The biggest of which was [jit.gl.lut](https://github.com/cju-media/jit.gl.lut). I started last night, which is probably why I did not write for *THE LIST* but thats besides the point. The abstraction wraps a haldCLUT processor shader that allows LUTs to be used in Max. I had the idea for this back in March or April but I finally got around to finally building it. I love the look of using LUTs. You can create some real cinematic looks from within Max. Especially since I often feature found footage, applying a LUT can add some uniformity to the footage. I also purposely made the LUT input in the object to be processable. Meaning not only can you use a LUT, but you can also process that LUT via jit.gl.pix to add additonal cool effects. The help patcher features an example of plane mapping three seperate LUTs. This means I am pulling one color channel from each LUT randomly. Thats what I love about Max. Doing something like that in another piece of software would be so time consuming and not able to change the LUT on the fly like I do here. 

I had a long discussion (maybe not long but in-depth) about why I use Max with my teacher. I had a lot of back and forth internally (between myself, not arguing with him) with why I am so adament about using Max. If I want do anything sequence like, I have to create a DAW from first principles everytime. If I want sequence type stuff, why am I going to use Max at all? Shouldn't I just be in a DAW from the start? But its the randomization that keeps bringing me back. Just like above and just like for this idea I have for a piece.

Since we have a multi-microphone record system for the organ, I had the idea of pulling feeds from those different mics and randomly switching between each stereo pair. The resulting effect is rapid EQ changes but with volume and space changes. It works well for our organ since it truly is spatial. Three seperate organs with five different cabinets. Mics placed all around the space gives a level of depth that changing the EQ just cannot do. Since we do multitrack recording for the services I was able to do a mockup with existing recordings. Its a subtle but present effect. If the stops were a little more strategically spaced out you will get a much more spatial effect.

My idea involves playing the organ via max, capturing the sound of the organ with the multiple mics, and then the patch will play them back, randomly switching between all of the mics. In the testing there are a few mics very close together (choir mics) so for the piece it would be better to choose just one of them and just have a more drastic difference overall. Of course add mics on the balconies to get the side organs. Then I will use MIDI sysex to control the stops of the organ. That the piece will have two layers of sound manipulation, via the stops and via the placement of the mics. 
...
I have been looking back at a lot of previous services. Theres a lot more growth in our quality than I expected. Before the lighting install it was SO dark on camera. Just anywhere that was not center chancel, the pulpit, or the lectern was just so dark. The new lights adds so much to the look of the service. The addition of the transition video for the sermon looks great. What I should really do is force transitions to organ in the shorter transitions. The only problem is that if there is piano the organist may be doing the transition there. I think with a real secondary setup I can control from I could manage blocking those transitions, maybe, big maybe. I could add video transitions every time but theres no telling how long a transition is going to be. We could have 3 minutes of transition or 5 seconds. Those transitions get real awkward when they're short. I have been watching some Kitchen Nightmares style church AV content and it really shows the level of quality we are providing the church for a reasonable cost. At the end of the day I am happy the product I am providing for the church. Its something I am really proud of.
